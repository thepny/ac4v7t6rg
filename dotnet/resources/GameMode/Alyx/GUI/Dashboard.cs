...
namespace Alyx.GUI
{
    class Dashboard : Script
    {
        ...
        #region Navigator
        [RemoteEvent("server::gonavigatorset")]
        public static void GoNavigatorSetCoord(Player player, int id)
        {
            switch (id)
            {
                case 5:
                    Vector3 waypoint = BusinessManager.getNearestBiz(player, 10);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 7:
                    waypoint = MoneySystem.ATM.GetNearestATM(player);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 8:
                    ...
                    return;
                case 9:
                    waypoint = BusinessManager.getNearestBiz(player, 7);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 10:
                    waypoint = BusinessManager.getNearestBiz(player, 9);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 11:
                    ...
                    return;
                case 12:
                    waypoint = BusinessManager.getNearestBiz(player, 2);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 13:
                    waypoint = BusinessManager.getNearestBiz(player, 6);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 14:
                    waypoint = BusinessManager.getNearestBiz(player, 11);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 20:
                    waypoint = new Vector3(-268.23242, -2031.9342, 29.025599);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 21:
                    waypoint = new Vector3(924.93176, 46.50941, 81.2);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 22:
                    waypoint = new Vector3(-743.1777, 5595, 40.53459);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 23:
                    waypoint = new Vector3(783.0346, -1867.8917, 29.325284);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 24:
                    waypoint = new Vector3(1210, -2987, 0);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 25:
                    waypoint = new Vector3(-1247.8358, -1473.2466, 3.1403518);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 26:
                    waypoint = new Vector3(-469.10852, -1717.8014, 17.56913);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 27:
                    waypoint = new Vector3(-1058.5121, -2538.0662, 13.94454);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 28:
                    waypoint = new Vector3(-914.9841, -2038.5116, 8.284984);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
                case 29:
                    ...
                    return;
                case 30:
                    waypoint = new Vector3(2193.4214, 5594.003, 52.639023);
                    Trigger.ClientEvent(player, "client::setposnavigator", waypoint.X, waypoint.Y, waypoint.Z);
                    return;
            }
        }
                    ...
                    case 0:
                        {// self inventory
                            ...
                            {//remove one item from player inventory
                                if (player.HasData("BUCKSPADESTATE") && player.GetData<bool>("BUCKSPADESTATE")) return;
                                if (item.Type == ItemType.BagWithDrill)
                                {
                                    SafeMain.dropDrillBag(player);
                                    return;
                                }
                                else if (item.Type == ItemType.Walkie)
                                {
                                    Walkie.CloseWalkieMenu(player);
                                }
