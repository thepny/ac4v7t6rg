    ...
    class WorkManager : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.Blip.CreateBlip(629, new Vector3(-260.96762, -904.54315, 31.190844), 0.8f, 4, Main.StringToU16("GoPostal"), 255, 0, true, 0, 0);
        ...
        public static SortedList<int, Vector3> Points = new SortedList<int, Vector3>
        {
            ...
            {9, new Vector3(473.9508, -1275.597, 29.60513) },  // AutoMechanic job
            {10, new Vector3(473.9508, -1275.597, 29.60513) },  // AutoMechanic job
        };
        ...
        {
            Trigger.ClientEvent(player, "NPC.cameraOn", "GOPOSTAL", 1000);
            if (Main.Players[player].WorkID != 2)
            {
                Trigger.ClientEvent(player, "client::opendialogmenu", true, "Джэймс Фостер", "Работа", "Приветсвую, хочешь порабоать почтальоном? Помоги мне немного тут, получишь деньги?", (new QuestAnswer("Как тут работать?", 20), new QuestAnswer("Устроиться", 21), new QuestAnswer("В другой раз", 2)));
                return;
            }
            else if (Main.Players[player].WorkID == 2 && player.HasData("ON_WORK") && player.GetData<bool>("ON_WORK") == false)
            {
                Trigger.ClientEvent(player, "client::opendialogmenu", true, "Джэймс Фостер", "Работа", "Приветсвую, хочешь порабоать почтальоном? Помоги мне немного тут, получишь деньги?", (new QuestAnswer("Как тут работать?", 20), new QuestAnswer("Уволиться", 21), new QuestAnswer("Начать работу", 22), new QuestAnswer("В другой раз", 2)));
                return;
            }
            else if (Main.Players[player].WorkID == 2 && player.HasData("ON_WORK") && player.GetData<bool>("ON_WORK") == true)
            {
                Trigger.ClientEvent(player, "client::opendialogmenu", true, "Джэймс Фостер", "Работа", "Приветсвую, хочешь порабоать почтальоном? Помоги мне немного тут, получишь деньги?", (new QuestAnswer("Как тут работать?", 20), new QuestAnswer("Уволиться", 21), new QuestAnswer("Закончить работу", 24), new QuestAnswer("Взять посылки", 23), new QuestAnswer("В другой раз", 2)));
                return;
            }
        }
        ...
        public static void callback_gpStartMenu(Player client, int id)
        {
            switch (id)
            {
                case 0:
                    ...
                    else Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не работаете почтальоном", 3000);
                    return;
                case 1:
                    {
                        if (Main.Players[client].WorkID != 2)
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не работаете почтальоном", 3000);
                            return;
                        }
