...
namespace Alyx.Fractions
{
    ...
    { // Revision 3.0
        ...
        public static void onResourceStart()
        {
            try
            {
                ...
                NAPI.Blip.CreateBlip(606, new Vector3(2517.026, -384.461, 93.14043), 1f, 4, Main.StringToU16("Federal Investigation Buerau"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(60, new Vector3(435.1756, -981.82, 29.56958), 0.8f, 4, Main.StringToU16("Los Santos Police Department"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(598, new Vector3(-2305.0386, 3388.69, 30.136518), 0.9f, 4, Main.StringToU16("San Andreas National Guard"), 255, 0, true, 0, 0);

                //NAPI.Blip.CreateBlip(455, Army.ArmyCheckpoints[2], 0.8f, 17, Main.StringToU16("Порт"), 255, 0, true, 0, 0);

                NAPI.Blip.CreateBlip(60, Manager.FractionSpawns[18], 0.8f, 16, Main.StringToU16("Los Santos Sheriff Departament"), 255, 0, true, 0, 0);

                NAPI.Blip.CreateBlip(543, Manager.FractionSpawns[1], 0.8f, 25, Main.StringToU16("The Families"), 255, 0, true, 0, 0);
                ...
                NAPI.Blip.CreateBlip(543, Manager.FractionSpawns[4], 0.8f, 42, Main.StringToU16("The Marabunta Grande"), 255, 0, true, 0, 0);
        ...
        public static SortedList<int, Vector3> FractionSpawns = new SortedList<int, Vector3>()
        {
            ...
            {9, new Vector3(2522.1992, -330.6218, 94.09237)},     // FBI 
            {10, new Vector3(1396.6372, 1156.9446, 113.213684)},     // La Cosa Nostra 
            {11, new Vector3(-1517.3071, 834.7842, 181.55157)},    // Russian Mafia
            {12, new Vector3(-1027.4393, 306.3273, 66.99371)},    // Мексиканская мафия
            {13, new Vector3(-1903.6112, 2081.1401, 139.2989)},    // Armenian Mafia 
            {14, new Vector3(-2339.0493, 3261.2566, 33.294193)},    // Army
            {15, new Vector3(-577.76764, -923.5319, 32.524772)},    // LSNews
            {16, new Vector3(944.8635, 6.0115, 74.7413)},    // NamanaBet
            {17, new Vector3(905.4962, -3200.6926, -97.10795)},    // Меривейзер
			{18, new Vector3(-454.6883, 6014.8755, 31.596548)}, // lssd Alyx
        ...
        public static void ClothesFractions(Player player, string id)
        {
            ...
            if (gender)
            {
                ...
                else if (Main.Players[player].FractionID == 15)
                {
                    switch (id)
                    {
                        case "Комплект WN #1":
                            player.SetClothes(11, 26, 6);
                            player.SetClothes(4, 35, 0);
                            player.SetClothes(6, 25, 0);
                            break;
                        case "Комплект WN #2":
                            player.SetClothes(11, 139, 1);
                ...
                else if (Main.Players[player].FractionID == 18)
                {
                    switch (id)
                    {
                        case "Комплект LSSD #1":
                            player.SetClothes(11, 26, 0);
                            player.SetClothes(3, Customization.CorrectTorso[Main.Players[player].Gender][26], 0);
                            player.SetClothes(4, 35, 0);
                            player.SetClothes(6, 10, 0);
        ...
        public static void sendFractionMessageChat(int fracid, string message)
        {
            var all_players = NAPI.Pools.GetAllPlayers();
            foreach (var p in all_players)
            {
                ...
                if (!Main.Players.ContainsKey(p)) continue;
                if (Main.Players[p].FractionID == fracid)
                    p.SendChatMessage("~b~ [Фракция] " + message);
                //NAPI.Notification.SendPictureNotificationToPlayer(p, message, pic, 0, 0, sender, submessage);
            }
        }
        public static void sendFractionSound(int fracid, string message)
        {
            var all_players = NAPI.Pools.GetAllPlayers();
            foreach (var p in all_players)
            {
                ...
                if (!Main.Players.ContainsKey(p)) continue;
                if (Main.Players[p].FractionID == fracid)
                    Trigger.ClientEvent(p, "client::playsoundurl", message);
            }
        }
    ...
    class Stocks : Script
    {
        ...
        private static Dictionary<int, Vector3> stockCoords = new Dictionary<int, Vector3>()
        {
            ...
            { 11, new Vector3(-15001.1862, 834.8209, 178.70326)}, // 234.63676
