            ...
            switch (NAPI.Data.GetEntityData(vehicle, "ACCESS"))
            {
                case "HOTEL":
                    ...
                    if (Core.VehicleStreaming.GetLockState(vehicle))
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы открыли двери транспорта", 3000);
                    }
                    else
                    {
                        Core.VehicleStreaming.SetLockStatus(vehicle, true);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы закрыли двери транспорта", 3000);
                    }
                    break;
                case "RENT":
            ...
            switch (NAPI.Data.GetEntityData(vehicle, "ACCESS"))
                    {
                        case "HOTEL":
                            ...
                            if (Core.VehicleStreaming.GetEngineState(vehicle))
                            {
                                ...
                                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы заглушили двигатель.", 3000);
                            }
                            else
                            {
                                Core.VehicleStreaming.SetEngineState(vehicle, true);
                                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы завели двигатель.", 3000);
                            }
                            break;
                        case "SCHOOL":
                        ...
                        case "beltCarPressed":
                            ...
                            else Commands.RPChat("me", sender, "отстегнул(а) ремень безопасности");
                        ...
                        case "RENT":
                            ...
                            {
                                Core.VehicleStreaming.SetEngineState(vehicle, false);
                                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы заглушили двигатель транспорта", 3000);
                            }
                            else
                            {
                                Core.VehicleStreaming.SetEngineState(vehicle, true);
