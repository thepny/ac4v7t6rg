...
namespace Alyx.Houses
{
    ...
    class House
    {
        ...
        public House(int id, string owner, int type, Vector3 position, int price, bool locked, int garageID, int bank, List<string> roommates)
        {
            ...
            marker = NAPI.Marker.CreateMarker(1, Position, new Vector3(), new Vector3(), 0.7f, new Color(0, 0, 0, 255), false, 0);
        ...
        {
            Trigger.ClientEvent(player, "client::ScreenFadeMotion");
            NAPI.Task.Run(() => {
        ...
        public void CreateInterior()
        {
            ...
            intmarker = NAPI.Marker.CreateMarker(1, HouseManager.HouseTypeList[Type].Position - new Vector3(0, 0, 0), new Vector3(), new Vector3(), 0.7f, new Color(67, 140, 239, 200), false, (uint)Dimension);
    ...
    class HouseManager : Script
    {
        ...
        {
            // name, position
            new HouseType("Автодом", new Vector3(1972.9766, 3816.2712, 32.30872), new Vector3(), 0.0f, "trevorstrailer"),
            new HouseType("Эконом", new Vector3(151.3818, -1007.6209, -100.11999), new Vector3(), 0.0f,"hei_hw1_blimp_interior_v_motel_mp_milo_"),
            new HouseType("Эконом+", new Vector3(266.053, -1007.3052, -102.12856), new Vector3(), 0.0f,"hei_hw1_blimp_interior_v_studio_lo_milo_"),
            ...
            new HouseType("Премиум", new Vector3(-17.666437, -589.20135, 88.99482), new Vector3(-38.84652, -578.466, 88.58952), 50.8f, "hei_hw1_blimp_interior_10_dlc_apart_high_new_milo_"),
            new HouseType("Элитный", new Vector3(-174.16713, 497.3336, 136.547), new Vector3(-164.9799, 480.7568, 137.1526), 40.0f, "apa_ch2_05e_interior_0_v_mp_stilts_b_milo_"),
        };
        ...
        [RemoteEvent("server_house_hackDoor_end")]
        public static void HouseHackDoorEnd(Player player, bool state)
        {
             if (state)
             {
                Houses.House house = Houses.FirstOrDefault(h => h.ID == player.GetData<int>("HOUSEID"));
             ...
             {
                Notify.Error(player, "Вы сломали отмычку");
                ...
                case 6:
                    {
                        ...
                        if (house == null) return;
                        Fractions.HijackingHouse.HijackingHouseData data = Fractions.HijackingHouse.HijackingHouseData.HijackingHouseDic.FirstOrDefault(i => i.Player == player);
                        if (data != null)
                        {
                            if (data.House == house.ID && data.TypeHouse == house.Type)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Сначал отнесите предмет", 3000);
                                    return;
                                }
                                if (house.Locked)
