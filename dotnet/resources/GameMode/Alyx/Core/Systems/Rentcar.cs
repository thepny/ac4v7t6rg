...
namespace Alyx.Core
{
    class Rentcar : Script
    {
        ...
        public static void OnResourceStart()
        {
            try
            {
                ...
                Log.Write($"Load {_rentAreas.Count} rent place");
            }
            catch (Exception e)
            {
    ...
    class RentArea
    {
        ...
        public RentArea(int iD, int type, Vector3 position, List<Vector3> spawnPositions, List<Vector3> spawnRotations)
        {
            ...
            _blip = NAPI.Blip.CreateBlip(RentAreaBlipSprites[(RentAreaType)Type], Position, 0.7f, 4, RentAreaBlipNames[(RentAreaType)Type], 255, 0, true, 0, 0);
        ...
        {
            { RentAreaType.Boats, 512},
            { RentAreaType.Helicopters, 512},
            { RentAreaType.Motorcycles, 512},
            { RentAreaType.Cycles, 512},
            { RentAreaType.Planes, 512},
            { RentAreaType.Sedans, 512},
            { RentAreaType.Sports, 512},
            { RentAreaType.OffRoad, 512}
        };
