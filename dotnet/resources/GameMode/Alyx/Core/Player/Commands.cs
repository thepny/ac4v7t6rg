...
namespace Alyx.Core
{
    class Commands : Script
    {
        ...
        [Command("takelic")] // Выдать лицензии (7 лвл)
        public static void CMD_takeLicense(Player player, int id, int lic)
        {
            try
            {
                if (!Group.CanUseCmd(player, "givelic")) return;
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Успешно забранно", 3000);
            }
            catch { }
        }
        ...
        [Command("alyxcoins")] // Выдать Alyx Coins (8 лвл)
        public static void CMD_Alyxcoins(Player player, int id, int amount)
        {
            try
            {
                if (!Group.CanUseCmd(player, "alyxcoins")) return;
        ...
        [Command("veh")]
        public static void CMD_createVehicle(Player player, string name = "zentorno", int a = 0, int b = 0)
        {
            try
            {
                if (player == null || !Main.Players.ContainsKey(player)) return;
        ...
        [Command("ssw")]
        public static void CMD_SetWheelSpeed(Player player, float speed)
        {
            Trigger.ClientEvent(player, "client::setspeedwheel", speed);
        }
        ...
        [Command("cka")] // Установить время (8 лвл)
        public static void CMD_AttachCarKey(Player player, float x, float y, float z, float xrot, float yrot, float zrot)
        {
           // player.PlayAnimation("anim@mp_player_intmenu@key_fob@", "fob_click", 49);
            BasicSync.DetachObject(player);
            BasicSync.AttachObjectToPlayer(player, NAPI.Util.GetHashKey("prop_buck_spade_09"), 57005, new Vector3(x, y, z), new Vector3(xrot, yrot, zrot));
        }
        ...
        public static void CMD_sendTargetToDemorgan(Player player, int id, int time, string reason)
        {
            try
            {
                if (Main.GetPlayerByID(id) == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Spieler mit ID nicht gefunden", 3000);
                    return;
                }
        ...
        [Command("offAlyxcoins")] // Выдать Alyx Coins в оффлайне (8 лвл)
        public static void CMD_offredbaks(Player client, string name, long amount)
        {
            if (!Group.CanUseCmd(client, "offAlyxcoins")) return;
            try
            {
                name = name.ToLower();
                ...
                GameLog.Admin(client.Name, $"offAlyxcoins({amount})", name);
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
        ...
        [Command("god")] // Админское бессмертие (2 лвл)
        public static void CMD_enableGodmode(Player player)
        {
            try
            {
                if (!Group.CanUseCmd(player, "god")) return;
        ...
        [Command("cro")]
        public static void CMD_CreateobJ(Player player, string name)
        {
            NAPI.Object.CreateObject(NAPI.Util.GetHashKey(name), player.Position, new Vector3(0, 0, 0), 255, 0);
        }
        [Command("rfp")]
