...
var slots = new Vue({
	...
	data: function () {
		return{
			server: 0,
			slotL: ["", "", 1, 0, 0, "нет", 0, 0, 0],
			slotM: ["", "", 1, 0, 0, "нет", 0, 0, 0],
			slotR: ["", "", 1, 0, 0, "нет", 0, 0, 0],
			...
			login: "username",
			currSlotId: 0,
			currSlot: ["", "", 1, 0, 0, "нет", 0, 0, 0],
			emptySlot: ["", "", 1, 0, 0, "нет", 0, 0, 0],
			...
			btnpressed: false,
			setspawnshow: false,
			dataspawns: [false,false,false],
			btns: [false,false,false,false],
			alllist: ["Место выхода", "Организация", "Дом"],
		};
	},
	methods: {
		deleteChar: () => {
		...
		set: function (data) {
			this.respawn = true
			bgshadow.addClass('opacity')
			removeallregs();
			this.dataspawns = JSON.parse(data)
        },
		btnsetspawncheck: function(id){
			if(this.btnpressed) return;
			if (this.btns[id] == true) return;
            ...
            this.respawn=false;
            this.respawn=true;
			mp.trigger('spawnchecksPos', id);
		},
		btnspawn: function() {
			mp.trigger('spawn::auth', this.selectspawn);
			setTimeout(() => {
				let mainblock = document.getElementById("authDivs");
...
document.addEventListener('keyup', function (event) {
    if (event.keyCode === keyCode) {
        if (pagestate == true) {
			pagestate = false
			setTimeout(() => {
				mainPage.addClass('closeopacity');
			}, 1200);
			mp.trigger('auth::fadescreen');
		}
    }
})
...
function enternextpage() {
	pagestate = false
	loadPage.addClass('closeopacity');
	setTimeout(() => {
...
$(document).ready(() => {
	...
	$(document)
		...
		.on('click', '.js-btn-back-create', () => {
			slots.disable_new_char_btn = false;
			slots.is_modal = false;
			slots.show_modal_new_char = false;
			return false;
		})
		...
		.on('click', '.choose-chars .choose-the', () => {
			if (slots.is_modal) return;
			U.trigger('selectChar', 1);
			return false;
		})
		.on('click', '.choose-chars .choose-the2', () => {
			if (slots.is_modal) return;
			U.trigger('selectChar', 2);
			return false;
		})
		.on('click', '.choose-chars .buy-the', () => {
			U.trigger1('buyNewSlot', 3);
			return false;
		})
